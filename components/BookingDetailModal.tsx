...
const InfoRow: React.FC<{ label: string, value: React.ReactNode }> = ({ label, value }) => (
    <div className="py-2 grid grid-cols-3 gap-4">
        ...
        <dd className="text-sm text-brand-text col-span-2">{value || '-'}</dd>
    </div>
);
...
export const BookingDetailModal: React.FC<BookingDetailModalProps> = ({ booking, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-brand-surface rounded-lg shadow-2xl p-8 max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start">
            <div>
                <h2 className="text-2xl font-bold text-brand-primary">Detalles de la Cita</h2>
                <p className="text-sm text-brand-text-secondary">ID: {booking.id.substring(0, 8)}</p>
            </div>
        ...
        <div className="mt-6 space-y-6">
            {/* Client Details */}
            <section>
                <h3 className="font-semibold text-brand-text border-b border-gray-700 pb-2 mb-2">Cliente</h3>
                <dl className="divide-y divide-gray-800">
                    <InfoRow label="Nombre" value={booking.customer.name} />
                    <InfoRow label="Email" value={<a href={`mailto:${booking.customer.email}`} className="text-brand-primary hover:underline">{booking.customer.email}</a>} />
            ...
            {/* Booking Details */}
            <section>
                <h3 className="font-semibold text-brand-text border-b border-gray-700 pb-2 mb-2">Cita</h3>
                <dl className="divide-y divide-gray-800">
                     ...
                     <InfoRow label="Total" value={<span className="font-bold text-brand-primary text-base">₲{totalPrice.toLocaleString('es-PY')}</span>} />
                </dl>
            </section>
        </div>
